...
test('SMS readiness preflight requires provider confirmations before enablement', async () => {
  const s = scenario();
  const result = await smsReadiness({ client: {}, functionUrl: 'https://x.twil.io', instanceId: 'home', smsEnabled: 'false', io: s.io, deps: s.deps });
...
test('SMS final readiness requires real delivery and HELP/STOP/START confirmations after enablement', async () => {
  const s = scenario({ answers: ['APPROVED', 'REGISTERED 0101', 'WEBHOOK', 'OUTBOUND', 'INBOUND', 'HELP', 'STOP BLOCKED', 'START RESTORED'] });
  const result = await smsReadiness({ client: {}, functionUrl: 'https://x.twil.io', smsEnabled: 'true', io: s.io, deps: s.deps });
...
test('SMS readiness fails closed on webhook drift or a missing confirmation', async () => {
  ...
  await assert.rejects(
    () => smsReadiness({ client: {}, functionUrl: 'https://x.twil.io', io: drift.io, deps: drift.deps }),
    /expected POST \/sms webhook/
  );

  const incomplete = scenario({ answers: ['APPROVED', 'not registered'] });
  await assert.rejects(
    () => smsReadiness({ client: {}, functionUrl: 'https://x.twil.io', io: incomplete.io, deps: incomplete.deps }),
