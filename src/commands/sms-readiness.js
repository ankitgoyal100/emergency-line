...
async function smsReadiness({ client, functionUrl, instanceId, smsEnabled, io, deps = tcDefault }) {
  ...
  if (!requireBooleanFlag('SMS_ENABLED', smsEnabled)) {
    ...
    io.info('Set SMS_ENABLED=true, redeploy, perform the real delivery/STOP/START/HELP tests, then run this command again for the final readiness gate.');
    return { ready: false, preflight: true, phoneNumber };
  }

  const deliveryChecks = [
    ['After confirming a real outbound application message was delivered to the intended recipient, type OUTBOUND: ', 'OUTBOUND'],
    ['After confirming a real inbound text was forwarded to the intended recipient, type INBOUND: ', 'INBOUND'],
